 ...
 * 把子项接进来）递归展平一遍。多棵树（不同工具的文件树）用的是同一份实现。 */
...
/** 一棵文件树要接进这套共用操作逻辑，需要提供的最小后端能力——不同工具/不同
 * 文件系统（本地/工作区/远程）签名形状可能不同，这里只要求调用方各自包一层
 * 适配。 */
 ...
 * 文件夹、按文件名过滤——多个工具/多棵文件树共用同一份实现，不各自重新写一遍。
 *
 * 故意不管的部分：具体怎么"列目录"（`children`/`expanded` 的状态形状）、
 * 拖拽移动、运行脚本、导入日志搜索、Diff 对比——这些要么两边状态管理方式
 * 本来就不同（前者），要么是特定工具场景特有的能力，硬塞进一个"万能 hook"
 * 只会增加一层没有共同受益者的抽象，各自在组件里按需要另外加。
 */
export function useFileTreeOperations({
  ...
  // 连续按 Shift 点几个不同的行，范围始终从同一个起点重新计算。
